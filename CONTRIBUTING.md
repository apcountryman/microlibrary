# Contributing to microlibrary

The following is a set of guidelines for contributing to microlibrary.
...
- The microlibrary version (Git commit SHA)
...
The `format.sh` script in the `ci` directory can be used to run `clang-format` on all C++
source code in the repository.
See the `format.sh` script's help text for usage details.
```shell
./ci/format.sh --help
```
