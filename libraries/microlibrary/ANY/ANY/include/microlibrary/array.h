...
/**
 * \file
 * \brief microlibrary::Array interface.
 */

#ifndef MICROLIBRARY_ARRAY_H
#define MICROLIBRARY_ARRAY_H
...
class Array {
  public:
     ...
     * \param[in] position The position of the array element to access.
     *
     * \pre position < microlibrary::Array::size()
     *
     * \return The element at the specified position in the array.
     */
    constexpr auto at( Position position ) noexcept -> Reference
    {
        MICROLIBRARY_EXPECT( position < size(), Generic_Error::OUT_OF_RANGE );
 ...
 *
 * \relatedalso microlibrary::Array
 *
...
{
    return ::microlibrary::equal( lhs.begin(), lhs.end(), rhs.begin() );
}
